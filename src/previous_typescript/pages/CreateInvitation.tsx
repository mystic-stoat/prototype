...
import {
  ChevronDown, ChevronRight, Save, Eye, Smartphone, Sparkles, Loader2,
  Heart, Calendar, MapPin, MessageSquareHeart, Mail, Palette, Image as ImageIcon,
...
const templates: Template[] = [
  {
    ...
    colors: { bg: "hsl(24, 26%, 92%)", text: "hsl(100, 10%, 20%)", accent: "hsl(100, 14%, 49%)" },
  ...
  },
  {
    ...
    colors: { bg: "hsl(350, 56%, 94%)", text: "hsl(350, 20%, 25%)", accent: "hsl(350, 80%, 72%)" },
    fontHeading: "'Playfair Display', serif",
    fontBody: "'DM Sans', sans-serif",
  ...
  },
  {
    ...
    colors: { bg: "hsl(0, 0%, 97%)", text: "hsl(0, 0%, 12%)", accent: "hsl(100, 14%, 49%)" },
  ...
  },
  {
    ...
    colors: { bg: "hsl(40, 30%, 95%)", text: "hsl(40, 10%, 18%)", accent: "hsl(40, 45%, 55%)" },
    fontHeading: "'Playfair Display', serif",
    fontBody: "'DM Sans', sans-serif",
  ...
  },
  {
    ...
    colors: { bg: "hsl(350, 56%, 96%)", text: "hsl(280, 10%, 30%)", accent: "hsl(350, 56%, 84%)" },
    fontHeading: "'Playfair Display', serif",
    fontBody: "'DM Sans', sans-serif",
  ...
  },
  {
    ...
    colors: { bg: "hsl(40, 20%, 14%)", text: "hsl(40, 30%, 90%)", accent: "hsl(43, 74%, 60%)" },
    fontHeading: "'Playfair Display', serif",
    fontBody: "'DM Sans', sans-serif",
...
const sections: SectionDef[] = [
  { key: "couple", title: "Couple Names", icon: Heart },
  { key: "datetime", title: "Date & Time", icon: Calendar },
  { key: "venue", title: "Venue", icon: MapPin },
  { key: "message", title: "Message", icon: MessageSquareHeart },
  { key: "rsvp", title: "RSVP", icon: Mail },
  { key: "theme", title: "Theme & Colors", icon: Palette },
  { key: "images", title: "Images", icon: ImageIcon },
];

// ─── Component ───────────────────────────────────────────────────────
const CreateInvitation = () => {
  const [selectedTemplate, setSelectedTemplate] = useState(templates[0]);
  const [expanded, setExpanded] = useState<Set<string>>(new Set(["couple"]));
              ...
              <div className="rounded-2xl border border-border/50 bg-card shadow-md shadow-foreground/[0.03] p-5 sm:p-6">
                ...
                <div className="space-y-3">
                  <Textarea
                    ...
                    placeholder="e.g. romantic beach wedding at sunset, rustic outdoor barn, luxury ballroom with gold accents…"
                    className="min-h-[80px] bg-background border-border/60 rounded-xl resize-none text-sm focus:ring-2 focus:ring-primary/20"
                  />
                  <Button
                  ...
                  >
                    {aiLoading ? (
                      ...
                      <><CheckCircle2 size={16} /> Generated!</>
                    ) : (
                      <><Sparkles size={16} /> Generate Invitation with AI</>
                    )}
                  </Button>
                </div>
              </div>
                  ...
                  {templates.map((tmpl) => {
                    ...
                    return (
                      <button
                        ...
                        className={`group relative rounded-xl border-2 p-3 text-left transition-all duration-200 active:scale-[0.97] ${
                          active
                            ? "border-primary bg-primary/5 shadow-md"
                            : "border-border/40 bg-background/60 hover:border-primary/40 hover:shadow-sm"
                        }`}
                      >
                        {/* Color swatch */}
                        <div className="flex gap-1.5 mb-2.5">
                          <span className="w-5 h-5 rounded-full border border-foreground/10" style={{ backgroundColor: tmpl.colors.bg }} />
                          <span className="w-5 h-5 rounded-full border border-foreground/10" style={{ backgroundColor: tmpl.colors.accent }} />
                          <span className="w-5 h-5 rounded-full border border-foreground/10" style={{ backgroundColor: tmpl.colors.text }} />
                        </div>
                    ...
                    <div key={sec.key}>
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-5 py-4 text-left transition-colors duration-150 ${
                          isOpen ? "bg-primary/[0.06]" : "hover:bg-muted/40"
                        }`}
                      >
                      ...
                      {isOpen && (
                        <div className="px-5 pb-5 pt-2 animate-fade-in space-y-3">
                          {sec.key === "couple" && (
                            <>
                              <div className="flex items-center justify-between mb-1">
                                <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Show section</Label>
                                <Switch checked={showSections.couple} onCheckedChange={(v) => setShowSections((p) => ({ ...p, couple: v }))} />
                              </div>
                              <div>
                          ...
                          {sec.key === "datetime" && (
                            <>
                              <div className="flex items-center justify-between mb-1">
                                <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Show section</Label>
                                <Switch checked={showSections.datetime} onCheckedChange={(v) => setShowSections((p) => ({ ...p, datetime: v }))} />
                              </div>
                              <div>
                          ...
                          {sec.key === "venue" && (
                            <>
                              <div className="flex items-center justify-between mb-1">
                                <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Show section</Label>
                                <Switch checked={showSections.venue} onCheckedChange={(v) => setShowSections((p) => ({ ...p, venue: v }))} />
                              </div>
                              <div>
                          ...
                          {sec.key === "message" && (
                            <>
                              <div className="flex items-center justify-between mb-1">
                                <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Show section</Label>
                                <Switch checked={showSections.message} onCheckedChange={(v) => setShowSections((p) => ({ ...p, message: v }))} />
                              </div>
                              <div>
                          ...
                          {sec.key === "rsvp" && (
                            <>
                              <div className="flex items-center justify-between mb-1">
                                <Label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Show section</Label>
                                <Switch checked={showSections.rsvp} onCheckedChange={(v) => setShowSections((p) => ({ ...p, rsvp: v }))} />
                              </div>
                              <div>
                              ...
                              <div className="flex gap-2">
                                {[t.colors.bg, t.colors.accent, t.colors.text].map((c, i) => (
                                  <div key={i} className="flex flex-col items-center gap-1">
                                    <span className="w-10 h-10 rounded-xl border border-foreground/10 shadow-sm" style={{ backgroundColor: c }} />
                          ...
                          {sec.key === "images" && (
                            <div className="flex flex-col items-center justify-center py-6 border-2 border-dashed border-border/50 rounded-xl bg-background/40">
...
const PhonePreview = ({
  ...
  <div className="flex flex-col items-center">
    <div className="relative">
      {/* Phone frame */}
      <div
        className="w-[300px] sm:w-[320px] h-[620px] sm:h-[640px] rounded-[2.5rem] border-[3px] border-foreground/80 shadow-2xl overflow-hidden relative"
      ...
      >
        ...
        <div className="h-full overflow-y-auto pt-10 pb-8 px-6 transition-all duration-500">
          {/* Greeting */}
          <p
            className="text-center text-xs tracking-widest uppercase mt-4 mb-2 transition-all duration-300"
            style={{ color: t.colors.accent, fontFamily: t.fontBody }}
          >
            {greeting}
          </p>
          ...
          {showSections.couple && (
            <h2
              className="text-center text-2xl sm:text-[1.7rem] font-bold leading-tight mb-4 transition-all duration-300"
          ...
          <div className="flex items-center justify-center gap-2 my-4">
            <span className="h-px w-10 opacity-30" style={{ backgroundColor: t.colors.text }} />
            <Heart size={14} style={{ color: t.colors.accent }} />
            <span className="h-px w-10 opacity-30" style={{ backgroundColor: t.colors.text }} />
          </div>

          {/* Message */}
          {showSections.message && (
            <p
              className="text-center text-xs leading-relaxed mb-5 transition-all duration-300"
              style={{ color: t.colors.text, fontFamily: t.fontBody, opacity: 0.75 }}
            >
              {message}
            </p>
          )}
          ...
          {/* Date & Time */}
          {showSections.datetime && (
            <div
              className="rounded-xl p-4 mb-4 text-center transition-all duration-300"
              style={{ backgroundColor: t.colors.accent + "18" }}
            >
              <div className="flex items-center justify-center gap-1.5 mb-1">
                <Calendar size={12} style={{ color: t.colors.accent }} />
                <span className="text-xs font-semibold tracking-wide" style={{ color: t.colors.text, fontFamily: t.fontBody }}>
                  {date}
                </span>
              </div>
              <div className="flex items-center justify-center gap-1.5">
                <Clock size={12} style={{ color: t.colors.accent }} />
                <span className="text-xs" style={{ color: t.colors.text, fontFamily: t.fontBody, opacity: 0.7 }}>
                  {time}
                </span>
              </div>
            </div>
          ...
          {/* Venue */}
          {showSections.venue && (
            <div className="text-center mb-5 transition-all duration-300">
              <div className="flex items-center justify-center gap-1.5 mb-1">
                <MapPin size={12} style={{ color: t.colors.accent }} />
                <span className="text-xs font-semibold" style={{ color: t.colors.text, fontFamily: t.fontBody }}>
                  {venue}
                </span>
              </div>
              <p className="text-[11px]" style={{ color: t.colors.text, fontFamily: t.fontBody, opacity: 0.55 }}>
                {venueAddress}
              </p>
            </div>
          )}
          ...
          {showSections.rsvp && (
            <div className="mt-4 transition-all duration-300">
              <div
                ...
                style={{ backgroundColor: t.colors.accent + "25" }}
              >
                <p className="text-[11px] font-medium tracking-wide" style={{ color: t.colors.text, fontFamily: t.fontBody }}>
                  {rsvpNote}
                </p>
              </div>
              <button
                className="w-full mt-3 rounded-xl py-2.5 text-xs font-semibold tracking-wide transition-opacity hover:opacity-90"
                style={{
